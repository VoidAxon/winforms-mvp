...
- **トーストの組み込みスタイル** — `ToastStyle.Soft` / `ToastStyle.Card`（角丸）を従来の `Default`（方角）に追加。`ToastOptions.Style` / `ToastDefaults.Style` で選択でき、カスタム `Renderer` は引き続きスタイルより優先。`public` な `SoftToastRenderer` / `CardToastRenderer`（配色・アイコン用の `protected virtual` フック付き）と、トースト窓を角丸にする `ToastRenderer.CornerRadius` を追加。
