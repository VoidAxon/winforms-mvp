...
```
MockPlatformServices            (プラットフォームサービスを束ねるコンテナ)
...
業務依存 (IXxxRepository 等)     (Presenter のコンストラクタに注入。プラットフォームサービスとは別経路)
```

プラットフォームサービスのモックは `WinformsMVP.Samples.Tests/Mocks/` 配下にあります。**業務依存 (Repository など) は `MockPlatformServices` には含まれません** — それらは Presenter のコンストラクタに直接注入します (下記参照)。
...
public class UserEditorPresenterTests
{
    ...
    public UserEditorPresenterTests()
    {
        ...
        // プラットフォームサービスはプロパティ経由、業務依存 (Repository) はコンストラクタ経由
    ...
    public void OnSave_WithValidData_ShowsSuccessMessage()
    {
        ...
        Assert.True(_platform.MessageService.HasCall(MessageType.Info, "saved"));
    }
}
```
...
> **業務依存はコンストラクタ、プラットフォームサービスはプロパティ。** `MockPlatformServices` が束ねるのは Message/Dialog/File/Navigator (+ Logger) の **プラットフォームサービス** だけです。`IUserRepository` のような業務依存は Presenter のコンストラクタに注入します (`new UserEditorPresenter(_repository)`)。実例は [`MainEmailPresenterTests.cs`](https://github.com/VoidAxon/winforms-mvp/blob/master/tests/WinformsMVP.Samples.Tests/Presenters/MainEmailPresenterTests.cs) の `new MainEmailPresenter(_mockRepository).WithPlatformServices(_mockServices)` を参照。
...
public void OnImport_WhenUserSelectsFile_ImportsContent()
{
    ...
    _platform.DialogProvider.OpenFileDialogResult = "C:\\users.csv";
...
{
    _platform.DialogProvider.OpenFileDialogResult = null;   // null/空 → Cancel をシミュレート
...
既定の `MockPlatformServices` は `NullLoggerFactory` を使うのでログ出力はありません (テストを汚さない)。ログ内容を検証したい場合は、エントリを記録する小さな `ILogger` を**自分で用意し**、`DefaultPlatformServices` 経由で差し込みます (テストプロジェクトに専用モックは同梱されていません)。
...
public void OnSave_LogsSuccess()
{
    ...
    var platform = new DefaultPlatformServices(null, loggerFactory);   // (register, loggerFactory)
