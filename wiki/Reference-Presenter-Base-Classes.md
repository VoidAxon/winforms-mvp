...
業務結果を返す Presenter は、上記いずれかに加えてマーカーインターフェイス `IRequestClose<TResult>` を実装します (詳細は [ウィンドウクローズモデル](Concept-Window-Closing-Model))。`IRequestClose<TResult>` にメンバーはありません — 結果型を宣言するだけで、`this.RequestClose(result, status)` 拡張メソッドが型安全に呼べるようになります。
...
- **ウィンドウの結果通知 (上向き・単発)** → `IRequestClose<TResult>` マーカーを実装し、`this.RequestClose(result, status)` を呼ぶ (✅ 出力ポート。フレームワークが `InteractionResult<TResult>` に変換して呼び出し元に返す)
...
public class GoodPresenter : WindowPresenterBase<IMyView>, IRequestClose<MyResult>
{
    ...
    private void OnSave()
    {
        ...
        this.RequestClose(result, InteractionStatus.Ok);   // Push close
    }
}
```
