...
> **アンカーは「インタラクションポイント」です。** `Point` を取らない形は、Windows が
> キーボード呼び出しのコンテキストメニュー (WM_CONTEXTMENU) に使うのと同じ慣例で
> 呼び出し時に解決されます:
>
> | 入力 | アンカー |
> |------|----------|
> | マウスクリック | クリック地点そのもの(カーソル位置 — grid のセルでも正確) |
> | キーボード(Enter/Space/ニーモニック) | **トリガーとなったコントロール**(例: Alt+D で押された Delete ボタン。ニーモニックはフォーカスを移さないため、フォーカスではなくトリガーを使います) |
> | キーボード(ショートカット等、トリガーが可視コントロールでない) | フォーカス中のコントロールの**現在の選択位置**(grid は現在セル、テキストボックスはキャレット、リストは選択項目;それ以外はコントロール矩形)— WM_CONTEXTMENU の「current selection に表示せよ」と同じ粒度 |
> | コントロール情報なし | アクティブウィンドウの中央 |
> | **コード起因**(タイマー / async 続き / 素の `Dispatcher.Dispatch`) | ウィンドウの中央。ただしコードから `PerformClick` した場合はそのコントロール。残留カーソル位置や無関係なフォーカスには**決して**アンカーしません(`Application.Idle` で「入力の同期処理チェーンが生きているか」を判定) |
> | アクティブウィンドウなし | 画面の中央(既定の MessageBox と同じ) |
>
> マウスかキーボードかは、`Application.AddMessageFilter`(マネージド API、interop なし)で
> 生の入力メッセージを観測して判定します(WM_KEYDOWN = キーボード、WM_*BUTTONDOWN = マウス)。
> アクションハンドラはトリガーとなった入力メッセージの処理中に同期実行されるため、
> 「最後の入力」= まさに今回のトリガーです。マウスがウィンドウ内に置きっぱなしでも
> ショートカットは正しくキーボードと判定されます。
>
> **同期呼び出しの契約:** UI スレッド上のイベント/アクションハンドラ内で同期に呼んで
> ください。`await` の後はインタラクション文脈が失われるため、非同期処理後の
> フィードバックには `Messages.ShowToast`(コーナートースト)を使ってください。
...
> **使い分けはルール 1 つだけ:** Presenter は**カーソル版**を使う(「クリックした場所に出す」—
> 座標の受け渡しは不要)。**`Point` 版は View 層コード向け** — Form が自分自身に
> `this.ShowToast("Saved", ToastType.Success, _saveButton.PointToScreen(...))` のように呼びます。
>
> ビジネス結果に依存しつつ特定位置に出したい稀なケースは、View に小さな意味メソッドを
> 1 つ生やします(例: `void ShowNameError(string message);` — 実装側がアンカーを決めて
> `Point` 版を呼ぶ)。それだけです。
