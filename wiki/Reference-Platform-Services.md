...
> ⚠️ **`MockPlatformServices`・各 `Mock*` サービス・`WithPlatformServices(...)` 拡張は、いずれもテストプロジェクト用のヘルパーで `WinformsMVP` パッケージには同梱されていません** (`tests/WinformsMVP.Samples.Tests/Mocks/`、`tests/WinformsMVP.Samples.Tests/TestHelpers/PresenterPlatformExtensions.cs`)。自分のテストプロジェクトにコピーして使ってください。`WithPlatformServices` は `internal` な `PresenterBase.SetPlatformServices` を `dynamic` 経由で呼ぶ実装なので、`InternalsVisibleTo` を持たない別アセンブリからでも動きます (直接 `SetPlatformServices` を呼ぶとアクセスレベルでコンパイルできません)。
