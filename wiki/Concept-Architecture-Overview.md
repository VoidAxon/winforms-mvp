...
| `ServiceLocator` + DI 連携 | サービスの構成・取得 | [Dependency Injection](Reference-DependencyInjection) |
...
Presenter からは、コンストラクタ注入なしで以下のサービスにアクセスできます。これらは `ServiceLocator.Current` (`IServiceProvider`) から遅延解決されます。
