...
- [4 つの選択肢](#4-つの選択肢)
- [選び方の決定木](#選び方の決定木)
- [選択肢 1: 親が子のメソッドを直接呼ぶ](#選択肢-1-親が子のメソッドを直接呼ぶ)
- [選択肢 2: 共有 Service + イベント](#選択肢-2-共有-service--イベント)
...
| **共有 Service + イベント** | 中 (Service 経由) | 状態を所有する責任が明確に存在するとき |
...
最も単純で結合度の強い方法。親 Presenter が子 Presenter (または子の View) のメソッドを直接呼びます。

### こんなときに

- 親が子を明確に制御する関係
- UserControl と親 Form のように、所有関係がはっきりしている
- 子は親の存在を知らなくてよい (一方向)

### 例: 設定パネルの値を MainPresenter が更新する
...
public class MainPresenter : WindowPresenterBase<IMainView>
{
    ...
    {
        _settingsPresenter = new SettingsPanelPresenter(View.SettingsPanel);
    }

    private void OnApplyTheme(string themeName)
    {
        // 親が子を直接更新
...
## 選択肢 2: 共有 Service + イベント

ステートを所有する **共有 Service** を用意し、Presenter は Service を介して連携します。**最も推奨される方法**。
...
public class ProductSelectorPresenter : ControlPresenterBase<IProductSelectorView>
{
    ...
    public ProductSelectorPresenter(IProductSelectorView view, IOrderManagementService orderService)
        : base(view)
    {
    ...
    {
        _orderService.AddProduct(View.SelectedProduct, View.Quantity);
    }
}
...
public class OrderSummaryPresenter : ControlPresenterBase<IOrderSummaryView>
{
    ...
    public OrderSummaryPresenter(IOrderSummaryView view, IOrderManagementService orderService)
        : base(view)
    {
        _orderService = orderService;
    }
    ...
    protected override void OnViewAttached()
    {
        _orderService.ProductAdded += OnProductAdded;
        _orderService.OrderCleared += OnOrderCleared;
    }

    private void OnProductAdded(object sender, ProductAddedEventArgs e)
    {
        View.OrderItems = _orderService.OrderItems;     // ← クエリも可能
        View.Total = _orderService.TotalAmount;
    }
...
✅ Presenter 間の結合がゼロ
✅ ステートの所有者が明確 (Service)
✅ クエリも可能 (`var items = _orderService.OrderItems`)
✅ ビジネスルールを Service に集約できる
✅ テストが楽 (Service をモックすればよい)
❌ Service の抽象化が必要
❌ Service インスタンスを DI コンテナで共有する必要がある
...
- ステートを所有する責任が特定の Service にない
- モジュール横断の通知 (例: 「ユーザーがログアウトしたら全 Presenter で表示をリセットしたい」)
- 親子関係も Service 関係もない Presenter 間の通信
...
// ✅ ステート管理は共有 Service、通知だけ EventAggregator (良い)
_orderService.AddProduct(product);                          // Service がステート所有
...
| 観点 | 直接呼び出し | 共有 Service | EventAggregator | コールバック |
|------|----------|-----------|---------------|----------|
| **結合度** | 強 | 中 | 弱 | 中 |
| **ステート管理** | ❌ | ✅ 推奨 | ❌ 不向き | ❌ |
| **通知 1 対 1** | ✅ | ✅ | ⚠️ 可能だが過剰 | ✅ |
| **通知 1 対 N** | ❌ | ✅ | ✅ 推奨 | ❌ |
| **クエリ可能** | ✅ | ✅ | ⚠️ Request パターン要 | ❌ |
| **デバッグしやすさ** | ✅ | ✅ | ❌ | ✅ |
| **テストしやすさ** | ✅ | ✅ | ✅ | ✅ |
| **モジュール横断** | ❌ | ⚠️ | ✅ | ❌ |
...
代わりに共有 Service か EventAggregator を使う。
...
通知は共有 Service 側に持たせる ([Presenter 基底クラス#公開-api-は最小限に保つ](Reference-Presenter-Base-Classes#公開-api-は最小限に保つ) 参照)。
...
// ✅ Good — ステートは Service、通知だけ EventAggregator
_cartService.AddProduct(product);                                  // Service が保存
...
- サンプル:
  - `src/WinformsMVP.Samples/ComplexInteractionDemo_ServiceBased/` — Service ベース
