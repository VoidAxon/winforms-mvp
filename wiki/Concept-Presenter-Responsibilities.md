...
| 関係 | 渡すもの | 機構 |
|------|---------|------|
| 親 → 子 | 命令 | 子のメソッドを直接呼ぶ (親はすでに子の参照を持つ。イベントは余計な間接) |
...
> ルールの詳細は [MVP 設計ルール](Design-Rules) の Rule 16 (Presenter メソッドの可視性) と Rule 17 (Presenter イベントの可視性) を参照してください。これらはフレームワークの Roslyn Analyzer によってコンパイル時に強制されます。
