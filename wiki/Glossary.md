...
- **`IWindowView : IActionableView, IWin32Window`** — Form (ウィンドウ) 用。`Closing` / `OnClosing` などウィンドウのライフサイクルを追加する。
...
本フレームワーク独自の `EventArgs` 派生クラス。`Reason` (`CloseReason` 列挙) と `Cancel` プロパティだけを持つ。`System.Windows.Forms.FormClosingEventArgs` に **対応する独自型** で、WinForms 型への参照は保持しない。`WindowNavigator` が `FormClosing` を **変換** してこのインスタンスを生成する。
