...
本フレームワーク独自の `enum`。`Normal` / `SystemShutdown` / `TaskManager` / `ParentClosing` / `Unknown` の値を持つ。`System.Windows.Forms.CloseReason` をラップしている。
...
`IPlatformServices` (=`ICommonServices`) の標準実装。`PlatformServices.Default` に通常はこのインスタンスをセットする。
...
`IRequestClose<TResult>`。業務結果を呼び出し元に返したい Presenter が実装するインターフェイス。`CloseRequested` イベントを 1 つ持つだけの最小契約。

詳細: [ウィンドウクローズモデル § IRequestClose](Concept-Window-Closing-Model#irequestclosetresult-の実装)
...
`InteractionStatus` enum。`Ok` / `Cancel` / `Error` の値を持つ。`IRequestClose<TResult>.CloseRequested` 等で使う。

### IViewBase / IWindowView

すべての View インターフェイスのマーカー。`IWindowView` は Form 用、`IViewBase` は UserControl 用。
...
`PlatformServices.Default` (静的プロパティ)。アプリ全体で使われる `ICommonServices` 実体を保持する。`Program.Main` で 1 回だけ設定する。
...
ウィンドウクローズモデルで、フレームワーク (`FormClosing`) が起点となる方向。`View.Closing` イベントで Presenter に伝わる。

詳細: [ウィンドウクローズモデル](Concept-Window-Closing-Model)

### Push (方向)

ウィンドウクローズモデルで、Presenter (`OnSave` / `OnCancel` 等) が起点となる方向。`IRequestClose<TResult>.CloseRequested` イベントで通知される。
...
本フレームワーク独自の `EventArgs` 派生クラス。`Reason` (`CloseReason` 列挙) と `Cancel` プロパティを持つ。`System.Windows.Forms.FormClosingEventArgs` をラップしている。
